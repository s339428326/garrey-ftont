...
import { updateOneArtwork } from '../features/userArtwork/userArtworkSlice';

const EditArtworkModel = ({ productData, isShow, setIsShow }) => {
  //上傳讀取判斷
  ...
  const [artworkImage, setArtworkImage] = useState(productData.productImgSrc);
  ...
  const {
    ...
    defaultValues: {
      ...
      bid_startPrice: productData.bid_startPrice,
    },
  });

  //model 開啟競標表單
  const [isBid, setIsBid] = useState(productData.isBid);

  //useFrom 無法管理input狀態
  const [tags, setTags] = useState(productData.tags);
  const [tagErrMsg, setTagErrMsg] = useState('');
  const [startDate, setStartDate] = useState(
    productData.startDate ? new Date(productData.startDate) : new Date()
  );
  const [endDate, setEndDate] = useState(
    productData.endDate ? new Date(productData.endDate) : new Date()
  );
  ...
  const onSubmit = async (value) => {
    ...
    console.log('test', productData.id, dataValue);
    dispatch(
      updateOneArtwork({
        id: productData.id,
        data: dataValue,
    ...
    setIsLoad(true);
    const sendData = await updateArtwork(productData.id, dataValue);
  ...
  return (
    <Model className={'w-1/2'} isShow={isShow}>
      <form onSubmit={handleSubmit(onSubmit)} action="">
        <div className="flex items-center gap-4 px-4 py-2 border-b">
          ...
          <button
            className={`p-2 ml-auto text-white  ${
              isLoad
                ? ' bg-gray-800'
        ...
        <div className="p-6">
          <div className="flex gap-4 mb-4">
            <div>
              <label className="" htmlFor={`"edit-${productData.name}`}>
                {artworkImage ? (
                  <div className="border-4 border-dashed  rounded-xl  w-[280px] h-[400px] p-2 object-contain flex justify-center items-center text-xl text-gray-400">
                    <img
              ...
              <input
                ...
                className="hidden"
                type="file"
                name={`"edit-${productData.name}`}
                id={`"edit-${productData.name}`}
              />
            </div>
